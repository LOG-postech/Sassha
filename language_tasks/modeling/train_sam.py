...
while True:
    ...
    if iter_num > rho_warmup_iters:
        ...
        while batch_buffer:
            pX, pY = batch_buffer.pop()
            
            if ddp:
                model.require_backward_grad_sync = not batch_buffer
            with ctx:
    ...
    if iter_num % log_interval == 0 and master_process:
        ...
        if iter_num > rho_warmup_iters:
            LL = len(optimizer.base_optimizer.state_dict()['state'])
            for jj in range(LL):
                momentum_norm += (optimizer.base_optimizer.state_dict()['state'][jj]['exp_avg'].detach().norm(2)) ** 2
