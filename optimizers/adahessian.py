# Acknowledgement: This code is based on https://github.com/davda54/ada-hessian/blob/master/ada_hessian.py
...
class Adahessian(torch.optim.Optimizer):
    ...
    At the previous experiment, I found there's a problem on 'denom' that the experiment result varies depending on 
    how to define 'denom'. So I decided to follow the official pytorch adam code.
    ...
    Arguments:
        ...
        lazy_hessian (int, optional) -- compute the hessian trace approximation only after *this* number of steps (to save time) (default: 1)
    ...
    def set_hessian(self):
        ...
        for p in filter(lambda p: p.grad is not None, self.get_params()):
            if self.state[p]["hessian step"] % self.lazy_hessian == 0:  # compute a new Hessian per `lazy_hessian` step
                params.append(p)
        ...
        if self.generator.device != params[0].device and self.seed is not None:  # hackish way of casting the generator to the right device
